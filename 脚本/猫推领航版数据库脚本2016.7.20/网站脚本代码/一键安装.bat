...
osql -E -i "%rootPath%3_2_代理库脚本.sql"
...
osql -d QPAgencyDB -E  -n -i "%rootPath%分页过程.sql"

osql -d QPAccountsDB -E  -n -i "%rootPath%切字符串.sql"
osql -d QPGameMatchDB -E  -n -i "%rootPath%切字符串.sql"
osql -d QPGameScoreDB -E  -n -i "%rootPath%切字符串.sql"
osql -d QPNativeWebDB -E  -n -i "%rootPath%切字符串.sql"
osql -d QPPlatformDB -E  -n -i "%rootPath%切字符串.sql"
osql -d QPPlatformManagerDB -E  -n -i "%rootPath%切字符串.sql"
osql -d QPRecordDB -E  -n -i "%rootPath%切字符串.sql"
osql -d QPTreasureDB -E  -n -i "%rootPath%切字符串.sql"
osql -d QPAgencyDB -E  -n -i "%rootPath%切字符串.sql"

set rootPath=3.存储过程\前台脚本\本地数据库\
osql -E -i "%rootPath%推荐游戏.sql"
osql -E -i "%rootPath%购买奖品.sql"
osql -E -i "%rootPath%清理日志.sql"
...
osql -E -i "%rootPath%金豆取款.sql"
osql -E -i "%rootPath%金豆存款.sql"
osql -E -i "%rootPath%金豆转账.sql"
osql -E -i "%rootPath%代理充值.sql"
...
osql -E -i "%rootPath%统计平台每天财富增减值.sql"
osql -E -i "%rootPath%赠送会员.sql"
osql -E -i "%rootPath%赠送经验.sql"
osql -E -i "%rootPath%赠送金豆.sql"
osql -E -i "%rootPath%赠送靓号.sql"
osql -E -i "%rootPath%每日统计.sql" 
osql -E -i "%rootPath%清理日志.sql" 

set rootPath=3.存储过程\后台脚本\金豆库\
osql -E -i "%rootPath%分页获取房间输赢.sql"
osql -E -i "%rootPath%财富游戏用户在线查询.sql"
osql -E -i "%rootPath%根据年月来统计代理充值情况.sql"
osql -E -i "%rootPath%抽奖记录时长.sql"
osql -E -i "%rootPath%抽奖设置.sql"
...
osql -E -i "%rootPath%玩家赢取金豆提醒.sql"
osql -E -i "%rootPath%后台充值提醒.sql"
osql -E -i "%rootPath%账户提款.sql"

set rootPath=3.存储过程\代理脚本\代理数据库\
osql -E -i "%rootPath%代理抽水比例.sql"
osql -E -i "%rootPath%代理设置.sql"
osql -E -i "%rootPath%代理提款.sql"
osql -E -i "%rootPath%代理转账.sql"
osql -E -i "%rootPath%发放有效会员奖励.sql"
osql -E -i "%rootPath%记录查询.sql"
osql -E -i "%rootPath%添加代理.sql"
osql -E -i "%rootPath%分页过程.sql"

set rootPath=4.创建作业\
osql -E -i "%rootPath%MT数据库作业创建.sql"
pause
