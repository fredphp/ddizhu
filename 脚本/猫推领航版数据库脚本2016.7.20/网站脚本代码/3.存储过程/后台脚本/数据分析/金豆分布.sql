...
BEGIN
	...
	-- 充值RMB总数
	SELECT @PayAmountTotal=SUM(PayAmount) FROM QPTreasureDBLink.QPTreasureDB.dbo.ShareDetailInfo
	
	-- 金豆总数
	SELECT @GoldTotal=SUM(Score+InsureScore) FROM GameScoreInfo 
	WHERE UserID NOT IN(SELECT UserID FROM QPAccountsDBLink.QPAccountsDB.dbo.AccountsInfo WHERE IsAndroid=1)
	...
	SELECT @RMBRate=StatusValue FROM QPAccountsDB.dbo.SystemStatusInfo WHERE StatusName='RateCurrency'
	SELECT @CurrencyRate=StatusValue FROM QPAccountsDB.dbo.SystemStatusInfo WHERE StatusName='RateGold'
	-- 返回参数
