...
DECLARE @CurrencyRate INT					-- 平台币与金豆的比例
...
BEGIN
	...
	-- 活跃用户数
	SELECT @ActiveUserCounts=COUNT(UserID) FROM QPTreasureDBLink.QPTreasureDB.dbo.StreamScoreInfo 
	...
	-- 最大、总充值金额
	SELECT @PayMaxAmount=MAX(PayAmount),@PayTotalAmount=SUM(PayAmount) FROM QPTreasureDBLink.QPTreasureDB.dbo.ShareDetailInfo
	
	-- 充值货币的充值金额
	SELECT @PayCurrencyAmount=MAX(PayAmount),@PayTotalAmount=SUM(PayAmount) FROM QPTreasureDBLink.QPTreasureDB.dbo.ShareDetailInfo
	
	-- 今日充值最高额度
	SELECT @CurrentDateMaxAmount=ISNULL(0,Max(PayAmount)) FROM QPTreasureDBLink.QPTreasureDB.dbo.ShareDetailInfo 
	WHERE ApplyDate>=CONVERT(VARCHAR(10),@CurrentTime,120) AND ApplyDate<DATEADD(dd,1,Convert(varchar(10),@CurrentTime,120))
	
	-- 充值总人数
	SELECT @PayUserCounts=COUNT(UserID) FROM (SELECT UserID FROM QPTreasureDBLink.QPTreasureDB.dbo.ShareDetailInfo GROUP BY UserID) AS A
	
	-- 二次充值玩家
	SELECT @PayTwoUserCounts=COUNT(Total) FROM (SELECT COUNT(UserID) AS Total 
	FROM QPTreasureDBLink.QPTreasureDB.dbo.ShareDetailInfo GROUP BY UserID) AS A WHERE Total>1
	
	-- 充值最多渠道
	--SELECT TOP 1 @MaxShareID=ShareID FROM QPTreasureDBLink.QPTreasureDB.dbo.ShareCollectInfo 
	...
	FROM (SELECT UserID FROM QPTreasureDBLink.QPTreasureDB.dbo.ShareDetailInfo GROUP BY UserID HAVING SUM(PayAmount)>=2000  ) AS A
	...
	-- 平台币总数
	SELECT @CurrencyTotal=SUM(Currency) FROM QPTreasureDBLink.QPTreasureDB.dbo.UserCurrencyInfo
	...
	-- 税收
	SELECT @GameTax=SUM(Revenue) FROM QPTreasureDBLink.QPTreasureDB.dbo.StreamScoreInfo
