...
BEGIN
	...
	SELECT COUNT(UserID) AS UserTotal,UserID,CONVERT(VARCHAR(7),LastCollectDate,120) AS StatDate,Sum(OnlineTimeCount) as TimeCount FROM StreamScoreInfo
	GROUP BY UserID,CONVERT(VARCHAR(7),LastCollectDate,120) HAVING Sum(OnlineTimeCount)>=@MonthActiveOnlineTime
