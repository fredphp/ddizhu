...
BEGIN
	...
	IF( (select count(0) from AccountsInfo where  datediff(d,getdate(),RegisterDate)=0  and   RegisterIP=@strClientIP )>@IPCount)
	BEGIN
	...
	-- 注册用户
	INSERT AccountsInfo (Accounts,Nickname,RegAccounts,LogonPass,InsurePass,DynamicPass,SpreaderID,Gender,FaceID,WebLogonTimes,RegisterIP,LastLogonIP,Compellation,PassPortID)
	VALUES (@strAccounts,@strNickname,@strAccounts,@strLogonPass,@strInsurePass,@strDynamicPass,0,@dwGender,@dwFaceID,1,@strClientIP,@strClientIP,@strCompellation,@strPassPortID)
