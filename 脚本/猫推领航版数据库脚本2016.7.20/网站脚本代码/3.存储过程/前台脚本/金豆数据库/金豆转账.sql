...
BEGIN
	...
	SELECT @TransferStauts=StatusValue FROM QPAccountsDBLink.QPAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'TransferStauts'
	IF @TransferStauts=0
	BEGIN
	...
	BEGIN
		SET @strErrorDescribe=N'抱歉的通知您！同一帐号不允许进行赠送金豆。'
		RETURN 1
	END
	...
	IF @DstNullity<>0
	BEGIN
		SET @strErrorDescribe=N'抱歉！您赠送的目标帐号暂时处于冻结状态，无法进行金豆赠送！'
		RETURN 202
	END	
	...
	IF @DstStunDown<>0
	BEGIN
		SET @strErrorDescribe=N'抱歉！您赠送的目标帐号使用了安全关闭功能，无法进行金豆赠送！'
		RETURN 203
	END
	--------------------------------------------------------------------------------
	...
	BEGIN
		SET @strErrorDescribe='您已经在金豆游戏房间了，需要进行转账操作，请先退出金豆游戏房间！'		
		RETURN 2
	END	
	...
	BEGIN
		SET @strErrorDescribe=N'非常抱歉,转账金豆数不能低于'+LTRIM(@TransferPrerequisite)
		RETURN 5
	END
	...
	BEGIN
		...
		BEGIN
			SET @strErrorDescribe=N'非常抱歉,转账后您保险柜内的余额不能少于'+LTRIM(@TransferRetention)+'金豆'
			RETURN 7
		END
	END
