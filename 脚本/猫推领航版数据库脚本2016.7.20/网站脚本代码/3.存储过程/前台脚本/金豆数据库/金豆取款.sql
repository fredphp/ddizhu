...
BEGIN
	...
	SELECT @MinSwapScore=StatusValue FROM QPAccountsDBLink.QPAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'BankPrerequisite'
	IF @MinSwapScore IS NULL
		SET @MinSwapScore=0

	IF @dwSwapScore < @MinSwapScore
	BEGIN
		SET @strErrorDescribe=N'非常抱歉,您每笔取出的数目至少为' + Convert(NVARCHAR(30), @MinSwapScore) + '金豆！'
		RETURN 6
	END
