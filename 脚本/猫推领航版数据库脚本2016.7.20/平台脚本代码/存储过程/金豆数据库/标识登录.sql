...
IF EXISTS (SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[dbo].[GSP_GR_EfficacyUserIDEx]') and OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[GSP_GR_EfficacyUserIDEx]
GO
...
IF EXISTS (SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[dbo].[GSP_GR_EfficacyMobileEx]') and OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[GSP_GR_EfficacyMobileEx]
GO
...
----------------add--------------
exec GSP_GR_WriteGameOneUserScore4Fish @dwUserID
----------------add--------------2015 11 12
	...
	BEGIN
		...
		-- 错误信息
		SET @strErrorDescribe=N'抱歉地通知你，您的金豆数据出现了异常情况，请联系客户服务中心了解详细情况！'
		RETURN 14
	END
	...
	BEGIN
		-- 锁定解除
		DELETE  QPTreasureDB.dbo.GameScoreLocker WHERE UserID=@dwUserID AND ServerID=@wServerID
	...
	BEGIN
		...
		BEGIN
			...
			-- 错误信息
			SET @strErrorDescribe=N'抱歉地通知你，金豆锁定操作失败，请联系客户服务中心了解详细情况！'
			RETURN 14
		END
	END
...
-- I D 登录
CREATE PROC [dbo].[GSP_GR_EfficacyUserIDEx]
	@dwUserID INT,								-- 用户 I D
	@strPassword NCHAR(32),						-- 用户密码
	@strClientIP NVARCHAR(15),					-- 连接地址
	@strMachineID NVARCHAR(32),					-- 机器标识
...
----------------add--------------2015 11 12
	...
	-- 加载玩家控制
	DECLARE @CtrlProb INT
	DECLARE @MainCtrlType INT
	DECLARE @CtrlType0 INT
	DECLARE @TargetScore BIGINT
	DECLARE @ServerWin BIGINT
	DECLARE @KindWin BIGINT
	DECLARE @strNote NVARCHAR(32)
	
	SELECT @CtrlProb=CtrlProb,@MainCtrlType=MainCtrlType,@CtrlType0=SubCtrlType0,@TargetScore=lTargetScore,@ServerWin=ServerID,@KindWin=KindID,@strNote=Note
	 FROM ControlUsers WHERE UserID=@dwUserID AND KindID=@wKindID AND ServerID=@wServerID
	 IF @CtrlProb IS NULL SET @CtrlProb=0
	 IF @MainCtrlType IS NULL SET @MainCtrlType=0
	 IF @CtrlType0 IS NULL SET @CtrlType0=0
	 IF @TargetScore IS NULL SET @TargetScore=0
	 IF @ServerWin IS NULL SET @ServerWin=0
	 IF @KindWin IS NULL SET @KindWin=0
	 IF @strNote IS NULL SET @strNote=''
	...
	SELECT @UserID AS UserID, @GameID AS GameID, @GroupID AS GroupID, @NickName AS NickName, @UnderWrite AS UnderWrite,  @FaceID AS FaceID,
		...
		@MasterOrder AS MasterOrder, @MemberOrder AS MemberOrder, @Score AS Score,  @Grade AS Grade, @Insure AS Insure,@UserMedal AS Ingot, @WinCount AS WinCount,
		@LostCount AS LostCount, @DrawCount AS DrawCount, @FleeCount AS FleeCount,@Experience AS Experience, @LoveLiness AS LoveLiness,@InoutIndex AS InoutIndex,
		@CtrlProb AS CtrlProb,@MainCtrlType AS MainCtrlType,@CtrlType0 AS CtrlType0,@TargetScore AS TargetScore,@ServerWin AS ServerWin,@KindWin AS KindWin,@strNote AS strNote
...
-- 手机登录
CREATE PROC [dbo].[GSP_GR_EfficacyMobileEx]
	@dwUserID INT,								-- 用户 I D
	@strPassword NCHAR(32),						-- 用户密码
	@strClientIP NVARCHAR(15),					-- 连接地址
	@strMachineID NVARCHAR(32),					-- 机器标识
