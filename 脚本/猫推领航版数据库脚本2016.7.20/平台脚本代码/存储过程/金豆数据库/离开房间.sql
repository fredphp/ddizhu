...
IF EXISTS (SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[dbo].[GSP_GR_LeaveGameServerEx]') and OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[GSP_GR_LeaveGameServerEx]
GO
...
BEGIN
	------------------------------------ 泡分开始 --------------------------------------------
	DECLARE @ServerID INT -- (-3:封顶配置,-2:积分房间公共配置,-1：金豆房间公共配置)
  ...
  if exists(select 1 from QPPlatformDB.dbo.GameGameItem where IsWriteScore=1 and GameID in (select GameID from QPPlatformDB.dbo.GameKindItem where KindID=@wKindID))
  	begin
		set @dwVariationPlayTimeCount=0
		set @lVariationExperience=0
        ...
        VALUES(@dwUserID,@wKindID,@wServerID,-1,@dwInoutIndex,@lVariationScore ,@lVariationGrade ,@lVariationRevenue,0,@dwVariationPlayTimeCount,SYSDATETIME())
		--set xact_abort off;
        set @lVariationFleeCount=case when @lVariationScore<>0 then 1 else 0 end-- 断线数目（改成以總分為判定標準）
        
		select @dwRecordPlayTimeCount=datediff(s,MIN(RequestDate),SYSDATETIME()),
		 @lVariationScore=sum(Score),@lVariationRevenue=sum(Revenue)
			,@lVariationWinCount=case when sum(Score)>0 then 1 else 0 end-- 胜利盘数（改成以總分為判定標準）
			,@lVariationLostCount=case when sum(Score)<0 then 1 else 0 end-- 失败盘数（改成以總分為判定標準）
			,@lVariationDrawCount=case sum(Score) when 0 then 1 else 0 end-- 和局盘数（改成以總分為判定標準）			
			,@dwVariationPlayTimeCount=sum(PlayTimeCount)+datediff(ss,MAX(RequestDate),SYSDATETIME())
		from RecordFishInfo
		where UserID=@dwUserID and KindID=@wKindID and ServerID=@wServerID
		group by UserID,KindID,ServerID;
		
		----------下面使用 删除前查询
		declare @SystemTimeConclude datetime  
		select @SystemTimeConclude=datediff(s,MIN(RequestDate),SYSDATETIME()) 
		  from RecordFishInfo		
		 where UserID=@dwUserID and KindID=@wKindID and ServerID=@wServerID		
	  group by UserID,KindID,ServerID;
		----------
		
		delete RecordFishInfo where UserID=@dwUserID and KindID=@wKindID and ServerID=@wServerID

----------------------------------------------------------------------------------------------------
		----------------------------------------------------------------------------------------------------
		if(@lVariationScore<>0 and exists(select 1 from QPPlatformDB.dbo.GameGameItem(nolock) where IsWriteScore=1 and GameID in (select GameID from QPPlatformDB.dbo.GameKindItem where KindID=@wKindID)))
		begin
			...
			if(@SystemTimeStart is null)
			set @SystemTimeStart=GETDATE()

			if(@SystemTimeConclude<'2016-01-01')
			set @SystemTimeConclude=GETDATE()
			
			-- 插入捕鱼类型写分类型 记录
			INSERT RecordDrawInfo(KindID,ServerID,TableID,UserCount,AndroidCount,Waste,Revenue,UserMedal,StartTime,ConcludeTime) 
			VALUES (@wKindID,@wServerID,0,1,1,-@lVariationScore,@lVariationRevenue,0,@SystemTimeStart,@SystemTimeConclude)

			-- 读取记录
			set @wDrawID=SCOPE_IDENTITY()
...
-- 离开房间
CREATE PROC [dbo].[GSP_GR_LeaveGameServerEx]
