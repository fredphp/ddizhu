...
IF EXISTS (SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[dbo].[GSP_GR_RecordTransfer]') and OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[GSP_GR_RecordTransfer]
GO
...
IF EXISTS (SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[dbo].[GSP_GR_UserInsureLogon]') and OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[GSP_GR_UserInsureLogon]
GO
...
-- 开通银行
CREATE PROC [dbo].[GSP_GR_UserEnableInsure]
	@dwUserID INT,								-- 用户 I D
...
-- 存入金豆
CREATE PROC [dbo].[GSP_GR_UserSaveScore]
	@dwUserID INT,								-- 用户 I D
	@cbActivityGame INT,
	@lSaveScore BIGINT,							-- 金豆数目
...
BEGIN
	...
	SELECT @UserID=UserID, @Nullity=Nullity, @StunDown=StunDown, @MoorMachine=MoorMachine, @MachineID=LastLogonMachine,@DynamicPass=DynamicPass
	...
	BEGIN
		...
		BEGIN
			SET @strErrorDescribe=N'抱歉，由于您的账号在其它地方登录导致验证令牌被重置，请重新登录平台后再次尝试！'
			RETURN 3
		END
	END
	...
	-- 查询元宝
	SELECT @lUserIngot=UserMedal FROM QPAccountsDB.dbo.AccountsInfo(NOLOCK) WHERE UserID=@dwUserID
	...
	SELECT @dwUserID AS UserID, @SourceScore AS SourceScore, @SourceInsure AS SourceInsure, @VariationScore AS VariationScore,
		@VariationInsure AS VariationInsure, @InsureRevenue AS InsureRevenue, @LockKindID AS KindID, @LockServerID AS ServerID,@lUserIngot AS lUserIngot
...
-- 提取金豆
CREATE PROC [dbo].[GSP_GR_UserTakeScore]
	@dwUserID INT,								-- 用户 I D
	@cbActivityGame INT,
	@lTakeScore BIGINT,							-- 金豆数目
...
BEGIN
	...
	DECLARE	@MachineID NVARCHAR(32)
	DECLARE @MoorMachine AS TINYINT
	SELECT @UserID=UserID, @InsurePass=InsurePass, @Nullity=Nullity, @StunDown=StunDown, @MoorMachine=MoorMachine, @MachineID=LastLogonMachine,@DynamicPass=DynamicPass
	FROM QPAccountsDBLink.QPAccountsDB.dbo.AccountsInfo WHERE UserID=@dwUserID
...
BEGIN
	...
	DECLARE @MoorMachine AS TINYINT
	SELECT @UserID=UserID, @InsurePass=InsurePass, @Nullity=Nullity, @StunDown=StunDown, @MoorMachine=MoorMachine, @MemberOrder=MemberOrder, @MachineID=LastLogonMachine,@DynamicPass=DynamicPass
	FROM QPAccountsDBLink.QPAccountsDB.dbo.AccountsInfo WHERE UserID=@dwUserID
...
--		SELECT @TargetUserID=UserID FROM QPAccountsDBLink.QPAccountsDB.dbo.AccountsInfo WHERE GameID=@strNickName	
	...
	BEGIN
		...
		BEGIN
			...
			SET @strErrorDescribe=N'非常抱歉,转账后您银行的余额不能少于'+LTRIM(@TransferRetention)+'金豆'
			RETURN 7
		END
	END
...
-- 查询银行
CREATE PROC [dbo].[GSP_GR_QueryUserInsureInfo]
	@dwUserID INT,								-- 用户 I D
	@strPassword NCHAR(32),						-- 用户密码
	@strClientIP NVARCHAR(15),					-- 连接地址
	@strErrorDescribe NVARCHAR(127) OUTPUT		-- 输出信息
...
BEGIN
	...
	SELECT @dwUserID AS UserID, @Score AS Score, @Insure AS Insure, @ServerID AS ServerID, @RevenueRateTake AS RevenueTake, 
		   ...
		   @EnjoinTransfer AS EnjoinTransfer,@lUserIngot AS lUserIngot
...
GO
----------------------------------------------------------------------------------------------------
CREATE PROC [dbo].[GSP_GR_RecordTransfer]
...
BEGIN
	...
	BEGIN
		...
		SELECT TOP 2000 a.RecordID as RecordID,b.GameID as SourceUserID,c.GameID as TargetUserID,b.NickName as SourceNickName,c.NickName as TargetNickName,a.SwapScore as SwapScore,a.CollectDate as CollectDate
		from (SELECT RecordID,SourceUserID,TargetUserID,SwapScore,CollectDate FROM RecordInsure WHERE TargetUserID<>0 AND TradeType=3 AND CollectDate>=@stTransferTime AND (SourceUserID=@dwUserID OR TargetUserID=@dwUserID)) AS a   
		INNER JOIN QPAccountsDBLink.QPAccountsDB.dbo.AccountsInfo AS b ON a.SourceUserID=b.UserID 
		INNER JOIN QPAccountsDBLink.QPAccountsDB.dbo.AccountsInfo AS c ON a.TargetUserID=c.UserID
    ...
    BEGIN
      ...
      SET @stTransferTimeEnd=DATEADD(day,1,@stTransferTimeStart)
		SELECT TOP 2000 a.RecordID as RecordID,b.GameID as SourceUserID,c.GameID as TargetUserID,b.NickName as SourceNickName,c.NickName as TargetNickName,a.SwapScore as SwapScore,a.CollectDate as CollectDate
		from (SELECT RecordID,SourceUserID,TargetUserID,SwapScore,CollectDate FROM RecordInsure WHERE TargetUserID<>0 AND TradeType=3 AND CollectDate>=@stTransferTimeStart AND CollectDate<@stTransferTimeEnd AND (SourceUserID=@dwUserID OR TargetUserID=@dwUserID)) AS a   
		INNER JOIN QPAccountsDBLink.QPAccountsDB.dbo.AccountsInfo AS b ON a.SourceUserID=b.UserID 
		INNER JOIN QPAccountsDBLink.QPAccountsDB.dbo.AccountsInfo AS c ON a.TargetUserID=c.UserID
        ORDER BY RecordID DESC
...
-- 修改密码
CREATE PROC [dbo].[GSP_GR_ModifyInsurePassword]
	@dwUserID INT,								-- 用户 I D
...
-- 登录银行
CREATE PROC [dbo].[GSP_GR_UserInsureLogon]
	@dwUserID INT,								-- 用户 I D
	@cbActivityGame INT,
	@strPassword NCHAR(32),						-- 用户密码
	@strClientIP NVARCHAR(15),					-- 连接地址
