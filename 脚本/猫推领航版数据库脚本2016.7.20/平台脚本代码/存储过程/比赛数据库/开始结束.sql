...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			-- 查询晋级玩家	
			SELECT MatchRank=ROW_NUMBER() OVER (ORDER BY Score DESC,WinCount,SignupTime ASC	),* INTO #RankUserList FROM MatchScoreInfo(NOLOCK) 	
			...
			ORDER BY Score DESC,WinCount,SignupTime ASC	
	...
	BEGIN
		-- 比赛配置
		DECLARE @cbPlayCount TINYINT
		SELECT @cbPlayCount=PlayCount FROM MatchImmediate(NOLOCK) WHERE MatchID=@dwMatchID AND MatchNo=1
			
		IF @cbPlayCount IS NOT NULL
		BEGIN
			-- 奖励名次			
			...
			-- 查询获奖玩家	
			SELECT MatchRank=ROW_NUMBER() OVER (ORDER BY Score DESC,WinCount,SignupTime ASC),* INTO #RankUserList1 FROM MatchScoreInfo(NOLOCK) 	
			WHERE ServerID=@wServerID AND MatchID=@dwMatchID AND MatchNo=@dwMatchNo AND (WinCount+LostCount+DrawCount)>=@cbPlayCount 
			ORDER BY Score DESC,WinCount,SignupTime ASC
