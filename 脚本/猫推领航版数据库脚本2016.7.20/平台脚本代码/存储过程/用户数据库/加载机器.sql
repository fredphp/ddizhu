...
BEGIN
	...
	WHERE UserID IN (SELECT TOP (@dwAndroidCount) UserID FROM AndroidLockInfo(nolock) WHERE AndroidStatus=0 AND ServerID=0 ORDER BY NEWID())		 

	-- 查询机器
	SELECT a.UserID,b.LogonPass INTO #TempAndroids FROM AndroidLockInfo(nolock) a,AccountsInfo(nolock) b
	...
	-- 查询机器
	SELECT * FROM #TempAndroids(nolock)
