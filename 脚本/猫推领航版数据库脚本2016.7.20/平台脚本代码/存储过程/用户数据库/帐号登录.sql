...
BEGIN
	...
	DECLARE @LoginErrorTimes INT
	DECLARE @RemindTimes INT
	DECLARE @LockTime INT
	
	SELECT @LoginErrorTimes=StatusValue FROM SystemStatusInfo WHERE StatusName='LoginErrorTimes'
	SELECT @RemindTimes=StatusValue FROM SystemStatusInfo WHERE StatusName='RemindTimes'
	SELECT @LockTime=StatusValue FROM SystemStatusInfo WHERE StatusName='LockTime'

	IF @LoginErrorTimes IS NULL SET @LoginErrorTimes=6
	IF @RemindTimes IS NULL SET @RemindTimes=3
	IF @LockTime IS NULL SET @LockTime=24

	DECLARE @ErrorTimes INT
	SELECT @ErrorTimes=ErrorTime FROM LoginErroe WHERE UserID=@UserID AND DATEDIFF(HH,LastErrorTime,GETDATE())<@LockTime
	IF @ErrorTimes IS NULL SET @ErrorTimes=0
	
	IF @ErrorTimes>=@LoginErrorTimes
	BEGIN
		SET @strErrorDescribe=N'登录密码错误次数过多，账号将会锁定'+LTRIM(STR(@LockTime))+'小时！'
		RETURN 3
	END
	...
	BEGIN
		...
		UPDATE LoginErroe SET ErrorTime=@ErrorTimes,LastErrorTime=GETDATE() WHERE UserID=@UserID
		IF @@ROWCOUNT=0
		BEGIN
			INSERT INTO LoginErroe(UserID,ErrorTime,LastErrorTime) VALUES(@UserID,@ErrorTimes,GETDATE())
		END
		...
		IF @ErrorTimes>=@RemindTimes
		BEGIN
			SET @strErrorDescribe=N'登录密码已错误'+LTRIM(STR(@ErrorTimes))+'次，还有'+LTRIM(STR(@LoginErrorTimes-@ErrorTimes))+'次机会！'
			RETURN 3
		END
		ELSE
		BEGIN
			SET @strErrorDescribe=N'您的密码输入有误，请查证后再次尝试登录！'
			RETURN 3
		END
	END
	...
	-- 删除密码错误次数
	DELETE LoginErroe WHERE UserID=@UserID
